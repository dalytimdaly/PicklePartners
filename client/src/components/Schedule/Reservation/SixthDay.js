...
useEffect(() => {
 ...
 const sixthDayfiltered6Court1 = sixthDayfiltered6.filter(result => result.court_number_id === 1).map(result => <div key={result.id} className={styles.reservedBlock}>
   ...
   <p className={result.size === 4 ? styles.userId : styles.userNone}>{result.user4_id ? <div>{renderNamePlayer4(result.user4_id)}</div> : <button className={styles.spotBtn} onClick={(event) => reserveSpotUser4(event)} value={result.id}>Spot Open</button>}</p>
   </p>
   </div>)
...
function renderNamePlayer4(param) {
  ...
  return (
    <div className={styles.pickleContainer}>
      ...
      <h4 className={styles.pageDate}>{format(day6, 'eee, MM/dd')}</h4>
      </div>
